...
- **Fleet/buffers** Instanced buffers are sized from the actual park, never hardcoded: `wheelsWanted === wheelsCount`, `unwrittenWheels === 0`, `unwrittenLenses === 0`.
  ...
  *(amended 2026-09-26)* When the multiplier is raised, a car that was hidden under the ground plane does **not** reappear at its `buildCars()` position. Over the idle seconds the active fleet has driven on and wrapped, so that old position may now sit inside an active vehicle — a same-axis overlap in one lane, which `leaderAhead` (sees itself), `bodyGap` (`dF + oF <= 0` skips same-axis) and `unstickCars` (perpendicular-only) all fail to resolve. `seatAtTail(c)` instead re-seats the returning car at the tail of its own lane (axis + `lineIdx` + `dir`), `MIN_SEAT_GAP = 4 m` behind the last active vehicle, and resets its crossing state (`xs`, `xGo`, `lead`, `speed`, `spin`). An empty lane is left untouched. The call sits inside `applyActiveCount()` — only manually re-enabled cars (`id >= activeCars`) go through it; the initial 52 keep the `buildCars()` anti-overlap placement.

  *(amended 2026-09-26, v2)* The first `seatAtTail` capped its search at `CROSS_LOOK * 2 = 132 m`; with 52 active cars spread over 36 lanes the nearest same-lane neighbour is often 400 m away, so `farthestAhead` stayed `-Infinity` and the returning car was left at its spawn point — inside an active vehicle. v2 drops the cap, adds `conflictsWithCrossAxis(c)` to catch perpendicular overlaps at junctions, and retries the seat up to 8 times in 20 m steps. If no free slot is found, `seatAtTail` returns `false` and `applyActiveCount` leaves the car inactive — `activeCars` is then recomputed from the real active count, so the HUD never reports more cars than the scene actually contains. Harness check 24 covers the round trip (`setTrafficMult(2.5)` then `(1.0)`, zero same-axis overlaps required).

  *(amended 2026-09-26, v3)* Two remaining flaws in the v2 seat search. First, `baseShift` used only the returning car's own `hl`; for two semi-trailers that is a 8 m shortfall and the car still lands inside its neighbour. v3 keeps the *car* found ahead (`farthestCar`) and uses `farthestAhead + farthestCar.hl + c.hl + MIN_SEAT_GAP`. Second, `applyActiveCount` walks cars by `id`, so when the multiplier goes up the car with the larger id was seated *before* seeing the smaller-id cars that activate in the same call — `76` and `112` in one lane is the reproducer. v3 activates cars in ascending `id` order, treats not-yet-active cars with `id < targetActive` as occupying their lane during the search, and `conflictsAt` now checks **both** the same-lane gap and the perpendicular OBB. `MAX_ATTEMPTS` grew to 12 with a 20 m step; a car that still finds no slot is left inactive and `activeCars` is recomputed from the real count.
