...
- **C-LANE-6** The traffic multiplier scales **time**, never the fleet: `animate()` calls `updateCars(dt * trafficMult)` while player and pedestrians keep real `dt`. Never re-implement it by adding/removing vehicles (fleet size is fixed at build time and every reference count in [harness.md](harness.md) depends on it).
