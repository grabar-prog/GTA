...
- **C-BOOT-1** *(amended 2026-09-25)* `boot()` order is fixed: renderer → `MeridianDayCycle.create(...)` → `MeridianTrafficAI.create(...)` → textures (facade/win/ground, via `dayCycle.makeXxx`) → `dayCycle.buildSky()` + `buildGround()` → `buildCity()` + `buildDistantCity()` → trees → streetlights → cars + `trafficAI.buildHeadlightPool()` + peds + character → minimap + `updateCycle(0)` + `renderer.compile(scene, camera)` → hide loader → `animate()`. Sub-rules: `updateCycle(0)` runs **before** the loader hides; textures come before any geometry that samples them; `dayCycle` and `trafficAI` exist before their first consumer (`buildSky`, `buildCars` respectively). Each step awaits `nextFrame()` so the bar repaints. `renderer.compile()` is the shader warm-up that keeps the first night from stuttering — all light-pool intensities are already established when it runs.
